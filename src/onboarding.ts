...
import {
  WalkthroughConfig, LLMProvider,
  GROQ_MODELS, OPENAI_MODELS, ANTHROPIC_MODELS,
} from "./config";

type OnSave = (cfg: WalkthroughConfig) => void;
type OnTest = (cfg: WalkthroughConfig) => Promise<{ ok: boolean; message: string }>;
...
export class OnboardingPanel {
  ...
  constructor(
    ...
    prefill: Partial<WalkthroughConfig>,
    onSave: OnSave,
    onTest: OnTest
  ) {
    ...
    this.panel.webview.onDidReceiveMessage(
      async (msg: { type: string; config?: WalkthroughConfig; url?: string }) => {
...
function buildHtml(prefill: Partial<WalkthroughConfig>): string {
  const groqJson     = JSON.stringify(GROQ_MODELS);
  const openaiJson   = JSON.stringify(OPENAI_MODELS);
  const anthropicJson= JSON.stringify(ANTHROPIC_MODELS);
