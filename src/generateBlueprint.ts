...
Return only the JSON object. Start with "{" and end with "}".`;
  ...
  const toStr = (v: any): string => {
    ...
    if (v && typeof v === 'object') return String(v.label ?? v.name ?? v.text ?? v.value ?? JSON.stringify(v));
    return String(v ?? '');
  };
  ...
  bp.scenes = (bp.scenes ?? []).map((s: any) => {
    ...
    if (s.type === 'error-flow') {
      if (Array.isArray(s.trySteps)) s.trySteps = s.trySteps.map(toStr);
      if (s.catchAction !== undefined) s.catchAction = toStr(s.catchAction);
    }
    ...
    if (s.type === 'conditional') {
      if (Array.isArray(s.truePath)) s.truePath = s.truePath.map(toStr);
      if (Array.isArray(s.falsePath)) s.falsePath = s.falsePath.map(toStr);
    }
    if (s.type === 'tree') {
      if (s.root !== undefined) s.root = toStr(s.root);
    }
    if (s.type === 'flow' && Array.isArray(s.steps)) {
      s.steps = s.steps.map((st: any) => typeof st === 'string' ? { label: st } : st);
    }
    return s;
  });
