...
| `src/session.ts` | `WalkthroughSession` — two-phase playback: Phase 1 narrates the whole file as one block (File Overview); Phase 2 is block-by-block through functions, entered only when D is pressed during Phase 1 (`inBlockMode = true`). Pause/resume (word-level + audio-trim resume), prefetch, subtitles, deep dive (line-by-line, only in Phase 2), Q&A (speaks answer). Video prefetch: `prefetchVideo(i)` awaits the audio buffer from `prefetchCache`, extracts WAV duration via `getWavDurationMs()`, passes it to `generateBlueprint` so video length matches audio exactly, then renders MP4 in background; `presentBlock` awaits the video promise (shows `🎬 Rendering animation...`) before playing audio so they start together. Narration text logged to output channel as `[script] [N/M] Label:\n<text>` immediately after generation. |
...
**Audio/video sync (source of truth):** `AnimationBlueprint.audioDurationMs` is the single source of truth — video duration is always derived from it, never the other way. Flow in `prefetchVideo`: await audio buffer → `getWavDurationMs(buffer)` (reads `byteRate` + `dataChunkSize` from WAV header, falls back to 8000ms) → `generateBlueprint(..., audioDurationMs)` → `renderBlockVideo`. In the Remotion layer: `Root.tsx` computes `durationInFrames = ceil(audioDurationMs/1000 × FPS)`; `CodeExplainer.tsx` computes `framesPerScene = floor(totalFrames / sceneCount)`. `motion/src/sampleBlueprint.ts` uses `audioDurationMs: 18000` (18 s) for studio preview.

**Blueprint scene count:** `minScenes = max(3, ceil(audioSecs/4))`, `maxScenes = min(8, minScenes+2)`. The LLM is instructed to split the narration into exactly `minScenes` equal chunks and generate one scene per chunk in story-arc order, with a `narrationChunk` field per scene (ignored by the renderer, useful for debugging). `bp.audioDurationMs` is always overwritten after LLM response to prevent drift.
