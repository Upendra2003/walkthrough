...
| `src/graphPanel.ts` | Unified right panel (WebviewPanel). **Layout:** LEFT col (65%) — `#video-zone` → `#flowchart-zone` (mutually exclusive, toggled by Deep Dive) → `#subtitle-section` → `#progress-track` → `#controls-bar`. RIGHT col (35%) — `#graph-section`. **Mermaid v11** CDN (`mermaid@11`), `securityLevel:'loose'`. **Deep Dive zone** (`#flowchart-zone`): header (badge + label + progress) → `#fc-step-panel` (dots + step title + description, above canvas) → canvas (SVG, pan+zoom) → footer (Generate Audio + Download). **Node click / step interaction:** `set-flowchart` message triggers `buildSteps(explanations)` + `renderDots()` → dot buttons appear in `#fc-step-panel`. `attachNodeClickHandlers()` wires click+hover on `g.node`, `g.actor`, `g.classGroup`, `.er.entityBox`, `.mindmap-node`, `g.statediagram-state`. Clicking a node calls `setActiveStep(idx)` → updates dot highlight, sets step text, calls `highlightNode()` which adds `.fc-node-active` (purple glow) to the SVG element via ID then text-content fallback. **Messages in:** `update`, `subtitle` (with `intervalMs`), `subtitle-loading`, `subtitle-hide`, `subtitle-language`, `set-paused`, `set-video-src`, `video-play` (with `delayMs`), `video-reset`, `enter-deepdive`, `exit-deepdive`, `flowchart-loading`, `set-flowchart` (with `mermaid` + `explanations`), `flowchart-end`, `deepdive-audio-ready`, `deepdive-audio-ended`. **Messages out:** `navigate`, `control` (`prev\|pause\|next\|deep-dive\|ask\|skip-file\|stop\|vol-<0-100>\|lang-<code>\|flowchart-next\|flowchart-prev\|flowchart-generate-audio:N\|next-file`). |
| `src/generateFlowchart.ts` | `generateFlowchart(code, blockLabel, language, cfg, explanationLang?)` → `Promise<FlowchartResult>`. Single LLM call returns JSON `{ mermaid, explanations }`. **6 diagram types:** `flowchart LR`, `sequenceDiagram`, `stateDiagram-v2`, `classDiagram`, `erDiagram`, `mindmap` — LLM picks the most appropriate. `explanationLang` (default `'en'`) appends "Return all explanation values in {lang}. Node IDs must remain in English." to the system prompt — node IDs never translated. `sanitizeMermaid()` applies type-specific fixes: mindmap `))shape((label` → `(label)`, sequence `(400)` → `[400]` in messages, flowchart arrow normalisation (`--->` / `->` → `-->`), reserved keyword node IDs renamed (`end` → `endNode`), `%%` comments stripped. `maxTokens=2200`. |
...
3. Posts `set-flowchart` → webview calls `buildSteps(explanations)` + `renderDots()`, then `mermaid.render()`, then `attachNodeClickHandlers()`
4. User clicks a node → matched to step index by ID/label → `setActiveStep(idx)` → highlights dot, updates `#fc-step-panel` text, calls `highlightNode()` (purple glow on SVG node)
5. D again → `exitDeepDive()` posts `exit-deepdive`, resolves `deepDiveExitResolve`, restores video mode
6. `skipFile()` (Next File button or F key) also posts `exit-deepdive` when in deepdive mode — prevents next file's video being hidden behind flowchart-zone
7. `playWithControls` returns `"next"` immediately if `mode === 'deepdive'` so audio never restarts
...
| Click any node / dot | Activate step: highlight node (purple glow) + show explanation in `#fc-step-panel` |
