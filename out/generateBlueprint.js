...
async function generateBlueprint(code, blockLabel, narration, _apiKey, audioDurationMs) {
    ...
    console.log(`[Blueprint] ${audioSecs.toFixed(1)}s audio → ` +
        `target ${targetScenes} scenes (min: ${minScenes}, max: ${maxScenes})`);
    const system = `You are an animation director for a code walkthrough tool. ` +
        `Given code and its narration, produce a motion graphics blueprint. ` +
        `Return ONLY valid JSON — no markdown fences, no explanation outside the JSON object.`;
    const user = `━━━ SCENE TYPE SCHEMAS (ALL fields are REQUIRED — no field may be omitted) ━━━
${SCENE_SCHEMAS}
...
Return only the JSON object. Start with "{" and end with "}".`;
    const text = await (0, narrate_1.callLLM)(system, user, 2500);
