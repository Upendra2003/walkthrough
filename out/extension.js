...
async function runMultiFileWalkthrough(extensionContext, rootUri, rootFileContext, sessionCfg) {
    ...
    activeGraphPanel.onControl((action) => {
        ...
        if (action.startsWith("lang-")) {
            ...
            if (sessionCfg)
                sessionCfg.language = code; // new sessions start in this language
            return;
        }
        // Flowchart deep dive controls
        if (action === "flowchart-next") {
            activeSession?.next();
            return;
        }
        if (action === "flowchart-prev") {
            activeSession?.prev();
            return;
        }
        if (action.startsWith("flowchart-generate-audio:")) {
            ...
            if (!isNaN(idx))
                activeSession?.generateDeepDiveAudio(idx);
            return;
        }
        if (action === "next-file") {
...
function apiHostname(cfg) {
    ...
    if (cfg.customBaseUrl) {
        try {
            return new URL(cfg.customBaseUrl).hostname;
        }
        catch { /* ignore */ }
    }
    return 'api.groq.com';
}
function checkConnectivity(hostname) {
...
function activate(context) {
    ...
    const explain = vscode.commands.registerCommand("walkthrough.explain", async () => {
        ...
        // ── Open panel immediately — full-width tab, no split ─────────────────────
        {
            ...
            activeGraphPanel = new graphPanel_1.GraphPanel(context, {
        ...
        if (!online) {
            ...
            vscode.window.showErrorMessage(`Walkthrough: Cannot reach ${host}. Check your internet connection and try again.`, "Retry").then(choice => {
                if (choice === "Retry")
                    vscode.commands.executeCommand("walkthrough.explain");
            });
            setRunning(false);
            return;
        }
